...
const candleColors = ['#BF2C34', '#4FB06D', '#45B7D1', '#F06857', '#BE398D' , '#5C62D6']
const balloonColors = ['#FF6B6B', '#4ECDC4', '#BE398D', '#', '#BF2C34' , '#F06857']
...
export default function BirthdayWish() {
  ...
  return (
    ...
    <div className="min-h-screen bg-purple-300 flex items-center justify-center p-4">
      ...
      >
        {/* Birthday card */}
        <Card className="mx-auto overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl border-2 border-black bg-gradient-to-br from-yellow-100 via-red-200 to-white">
          {/* Card header with birthday message */}
          <CardHeader className="text-center">
            <CardTitle className="text-4xl font-bold text-red-900">Happy 18th Birthday  in advance!</CardTitle>
            <CardDescription className="text-2xl font-semibold text-blue-800  ">Yusra Saleem</CardDescription>
            <p className="text-lg text-gray-700 font-bold">November 7th</p>
          </CardHeader>
                ...
                {[...Array(totalCandles)].map((_, index) => (
                  <AnimatePresence key={index}>
                    ...
                    {(celebrating && index <= candlesLit) || (!celebrating && index < candlesLit) ? (
                      ...
                      // Unlit candle
                      <FaBirthdayCake
                        className={`w-8 h-8 text-black transition-colors duration-300 ease-in-out cursor-pointer hover:scale-110`}
                        onClick={() => lightCandle(index)}
                      />
                    )}
                  </AnimatePresence>
          ...
          <CardFooter className="flex justify-center">
            <Button 
              className="bg-black text-white hover:bg-pink-400 transition-all duration-300"
              onClick={celebrate}
              disabled={celebrating}
            >
              Let Celebrate! <FaGift className="ml-2 h-4 w-4" />
            </Button>
          </CardFooter>
        </Card>
      </motion.div>
